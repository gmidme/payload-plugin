...
public class GroupingMixupRescorerBuilder extends RescorerBuilder<GroupingMixupRescorerBuilder> {
    ...
    protected RescoreContext innerBuildContext(int i, QueryShardContext queryShardContext) throws IOException {
        IndexFieldData<?> groupingField =
                this.groupByField == null ? null : queryShardContext.getForField(queryShardContext.getFieldType(this.groupByField));
        ScoreScript.LeafFactory scriptFactory = queryShardContext.compile(rescoreScript, ScoreScript.CONTEXT)
                .newFactory(rescoreScript.getParams(), queryShardContext.lookup());
